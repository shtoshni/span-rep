...
class TaskModel(nn.Module):
    def __init__(self, encoder,
                 span_dim=256, pool_method='avg', num_labels=1, num_spans=2,
                 **kwargs):
        super(TaskModel, self).__init__()
        self.encoder = encoder
        try:
            # BERT, SpanBERT, RoBERTa
            self.embedding_layer = self.encoder.model.embeddings.word_embeddings
        except AttributeError:
            try:
                # XLNet
                self.embedding_layer = self.encoder.model.word_embedding
            except AttributeError:
                # GPT2
                self.embedding_layer = self.encoder.model.wte
        ...
        # This module operates at the word embeddings directly
    ...
    def forward(self, batch_data):
        ...
        embedded_inp = self.embedding_layer(text.cuda())
        s2_repr = self.calc_span_repr(embedded_inp, batch_data.span2.cuda(), index='1')
